...
__export(src_exports, {
  ...
  buildSignInMessage: () => buildSignInMessage,
  createAppClient: () => createAppClient,
  createChannel: () => createChannel,
  createClient: () => createClient,
  createWalletClient: () => createWalletClient,
  generateNonce: () => import_siwe3.generateNonce,
  get: () => get,
  parseSignInURI: () => parseSignInURI2,
  poll: () => poll,
  post: () => post,
  status: () => status,
  unwrap: () => unwrap,
  verifySignInMessage: () => verifySignInMessage,
  viemConnector: () => viemConnector,
  watchStatus: () => watchStatus
});
module.exports = __toCommonJS(src_exports);
var import_siwe3 = require("siwe");
...
var get = async (client, path5, opts) => {
  return import_neverthrow.ResultAsync.fromPromise(
    ...
    return import_neverthrow.ResultAsync.fromPromise(response.json(), (e) => {
      return new AuthClientError("unknown", e);
    }).andThen((data) => {
      return (0, import_neverthrow.ok)({ response, data });
    });
  });
};
  ...
  while (Date.now() < deadline) {
    ...
    if (res.isOk()) {
      ...
      if (response.status === successCode) {
        return (0, import_neverthrow.ok)(res.value);
      }
      onResponse(res.value);
      await new Promise((resolve) => setTimeout(resolve, interval));
    } else {
      return (0, import_neverthrow.err)(res.error);
    }
  }
  return (0, import_neverthrow.err)(new AuthClientError("unavailable", `Polling timed out after ${timeout}ms`));
};
...
var validate = (params) => {
  return import_neverthrow2.Result.fromThrowable(
    ...
    () => new import_siwe.SiweMessage(params),
...
var parseResources = (message) => {
  ...
  if (fid.isErr())
    return (0, import_neverthrow2.err)(fid.error);
  return (0, import_neverthrow2.ok)({ fid: fid.value });
};
var parseFid = (message) => {
  ...
  if (!resource) {
    return (0, import_neverthrow2.err)(new AuthClientError("bad_request.validation_failure", "No fid resource provided"));
  }
  const fid = parseInt(resource.match(FID_URI_REGEX)?.[1] ?? "");
  if (isNaN(fid)) {
    return (0, import_neverthrow2.err)(new AuthClientError("bad_request.validation_failure", "Invalid fid"));
  }
  return (0, import_neverthrow2.ok)(fid);
};
var validateStatement = (message) => {
  const validStatement = message.statement === STATEMENT || message.statement === "Farcaster Connect";
  if (!validStatement) {
    return (0, import_neverthrow2.err)(new AuthClientError("bad_request.validation_failure", `Statement must be '${STATEMENT}'`));
  }
  return (0, import_neverthrow2.ok)(message);
};
var validateChainId = (message) => {
  if (message.chainId !== CHAIN_ID) {
    return (0, import_neverthrow2.err)(new AuthClientError("bad_request.validation_failure", `Chain ID must be ${CHAIN_ID}`));
  }
  return (0, import_neverthrow2.ok)(message);
};
var validateResources = (message) => {
  ...
  if (fidResources.length === 0) {
    ...
    return (0, import_neverthrow2.err)(new AuthClientError("bad_request.validation_failure", "Multiple fid resources provided"));
  } else {
    return (0, import_neverthrow2.ok)(message);
  }
};
...
var verify = async (nonce, domain, message, signature, options = {
  ...
  if (valid.isErr())
    return (0, import_neverthrow3.err)(valid.error);
  const siwe = (await verifySiweMessage(valid.value, signature, provider)).andThen(mergeResources);
  if (siwe.isErr())
    return (0, import_neverthrow3.err)(siwe.error);
  if (!siwe.value.success) {
    const errMessage = siwe.value.error?.type ?? "Failed to verify SIWE message";
    return (0, import_neverthrow3.err)(new AuthClientError("unauthorized", errMessage));
  }
  const fid = await verifyFidOwner(siwe.value, getFid);
  if (fid.isErr())
    return (0, import_neverthrow3.err)(fid.error);
  if (!fid.value.success) {
    const errMessage = siwe.value.error?.type ?? "Failed to validate fid owner";
    return (0, import_neverthrow3.err)(new AuthClientError("unauthorized", errMessage));
  }
  const { error, ...response } = fid.value;
  return (0, import_neverthrow3.ok)(response);
};
var validateNonce = (message, nonce) => {
  if (message.nonce !== nonce) {
    return (0, import_neverthrow3.err)(new AuthClientError("unauthorized", "Invalid nonce"));
  } else {
    return (0, import_neverthrow3.ok)(message);
  }
};
var validateDomain = (message, domain) => {
  if (message.domain !== domain) {
    return (0, import_neverthrow3.err)(new AuthClientError("unauthorized", "Invalid domain"));
  } else {
    return (0, import_neverthrow3.ok)(message);
  }
};
var verifySiweMessage = async (message, signature, provider) => {
  return import_neverthrow3.ResultAsync.fromPromise(message.verify({ signature }, { provider, suppressExceptions: true }), (e) => {
    return new AuthClientError("unauthorized", e);
  });
};
var verifyFidOwner = async (response, fidVerifier) => {
  const signer = response.data.address;
  return import_neverthrow3.ResultAsync.fromPromise(fidVerifier(signer), (e) => {
...
var mergeResources = (response) => {
  return parseResources(response.data).andThen((resources) => {
    return (0, import_neverthrow3.ok)({ ...resources, ...response });
  });
};
...
var parseSignInURI = (signInUri) => {
  ...
  if (!channelToken) {
    return (0, import_neverthrow4.err)(validationFail("No channel token provided"));
  }
  if (!params["nonce"]) {
...
var build = (params) => {
  ...
  if (valid.isErr())
    return (0, import_neverthrow5.err)(valid.error);
  else {
    const siweMessage = valid.value;
    return (0, import_neverthrow5.ok)({ siweMessage, message: siweMessage.toMessage() });
  }
};
...
var viemConnector = (args) => {
  const publicClient = (0, import_viem.createPublicClient)({
    ...
    transport: (0, import_viem.http)(args?.rpcUrl)
  });
