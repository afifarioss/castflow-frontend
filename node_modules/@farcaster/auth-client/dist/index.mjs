...
var parseSignInURI = (signInUri) => {
  ...
  if (!channelToken) {
    return err4(validationFail("No channel token provided"));
  }
  if (!params["nonce"]) {
    return err4(validationFail("No nonce provided"));
  }
  if (!params["siweUri"]) {
    return err4(validationFail("No SIWE URI provided"));
  }
  if (!params["domain"]) {
    return err4(validationFail("No domain provided"));
  }
  const { siweUri, ...siweParams } = params;
  return ok4({ channelToken, params: { uri: siweUri, ...siweParams } });
};
...
var build = (params) => {
  ...
  if (valid.isErr())
    return err5(valid.error);
  else {
    const siweMessage = valid.value;
    return ok5({ siweMessage, message: siweMessage.toMessage() });
  }
};
